...
```tsx
import { useFetchUserFollowersCount } from "replyke";
...
function FollowerCountDisplay({ userId }: { userId: string }) {
  ...
  return <button onClick={handleFetchFollowersCount}>Get Followers Count</button>;
}
```
...
|--------------|-------------------|-------------------------------------------|
| `count`      | `{ count: number }` | An object containing the number of followers. |
