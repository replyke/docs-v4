...
```tsx
import { useRequestNewAccessToken } from "replyke";
...
| --------------- | --------- | -------- | ----------- |
| `refreshToken`  | `string`  | Yes      | The refresh token for the user session. React Native apps must pass this token explicitly, as it is not managed automatically. React apps handle this via cookies and do not need to provide this token explicitly. |
