...
```tsx
import { useInfusedData } from "replyke";
...
function InfusedEntityList({ entities }: { entities: Entity[] }) {
  const infusedEntities = useInfusedData({
    ...
    infuseData: async (referenceId) => {
      ...
      const response = await fetch(`https://api.example.com/details/${referenceId}`);
...
| `infuseData` | `(referenceId: string) => Promise<Record<string, any> \| null>` | No       | A function to fetch external data for an entity by its `referenceId`. |
