...
```tsx
import { useEntityVotes } from "replyke";
...
function EntityVoteButtons({ entity, setEntity }: { entity: Entity; setEntity: React.Dispatch<React.SetStateAction<Entity>> }) {
  const {
    ...
    loginRequiredCallback: () => alert("Please log in to vote."),
    usernameRequiredCallback: () => alert("Please set a username to vote."),
    entity,
    setEntity,
  });
...
|--------------------------|-----------------------------------------|----------|----------------------------------------------------------------------------|
| `loginRequiredCallback`  | `() => void`                           | Yes      | Callback function triggered when a user is not logged in.                 |
| `usernameRequiredCallback` | `() => void`                         | No       | Callback function triggered when a user does not have a username.         |
