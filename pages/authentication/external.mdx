...
-   When a logged-in user is detected, the app makes a request to the developer’s server (not to Replyke's server, but to your own) to obtain a signed JWT.
...
import { AuthProvider, ReplykeProvider } from "replyke";
