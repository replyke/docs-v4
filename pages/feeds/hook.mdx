...
26.  **`loadMore`**: Fetches additional entities for the feed.
27.  **`resetEntities`**: Resets the feed, refreshing data without altering filter states.
28.  **`createEntity`**: Creates a new entity within the feed context. The newly created entity is automatically added to the feed.
29.  **`deleteEntity`**: Deletes an entity within the feed context. The deleted entity is automatically removed from the feed.
