...
This object can be provided directly to the `FeedProvider` component to apply static location filtering during initialization. To dynamically adjust the location filters after initialization, the `useFeed` hook provides the `setLocationFilters` method.
...
```tsx
import { FeedProvider } from 'replyke';
...
To change the location filter dynamically, use the `useFeed` hook:
...
#### Conclusion
Location Filters provide a powerful way to enhance feed relevance by focusing on geographic proximity. Whether used statically during initialization or dynamically via the `useFeed` hook, this feature enables users to tailor their feeds for a highly localized experience. Combine it with other filters like Keywords or Title Filters for even more precise content curation.
