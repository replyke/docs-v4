...
The `createIfNotFound` flag instructs Replyke to create an entity if one isn't found. This is particularly useful for integration with existing datasets or static apps. When used correctly, this flag will only cause the creation of an entity once per item or entity. Subsequent requests will locate the previously created entity.
...
#### Note:
Avoid nesting multiple `EntityProvider` components for the same content. Doing so can result in unexpected behavior.
